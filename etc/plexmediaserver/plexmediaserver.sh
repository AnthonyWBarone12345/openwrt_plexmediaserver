...
PLEX_BROWSER_ROOT_default='/mnt/BaroneHouse'
...
unpackme() {
  ...
  echo "Preparing Plex for Use - Extracting Plex Binaries and Library Files"
...
startme() {
  ...
  [ "$(pwd)" != "${PLEX_BIN_DIR}" ] && echo "Something went wrong, not able to switch to ${PLEX_BIN_DIR}. Aborting plex startup." && exit 1
...
#  export PLEX_MEDIA_SERVER_INFO_PLATFORM_VERSION="OpenWRT"
  ...
  case "${inArg}" in
    ...
    *)
      echo "plexmediaserver.sh needs one of the folling options (unpack|start|stop|restart)"
  esac
 esac
done
